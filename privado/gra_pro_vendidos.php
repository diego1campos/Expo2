<?php
	...
	//Se llama a la funcion header que pone todos los css y para todos los .php de publico ='}
  	$permisos = Page::header("Productos más vendidos", "graficos_reportes");
  	
  	if ( $permisos == 0 ) header( "location: index" );

	            				/*tratado de sacar los productos mas vendidos*/
	            				//SUMA TODO ='DDD--> SELECT SUM( detalles_pedidos.cantidad_producto ) + sum( detalles_pedidos_local.cantidad_producto ) SUMATODO FROM `detalles_pedidos`, `detalles_pedidos_local`

	            				
	            				$productos = Database::getRows( "SELECT DISTINCT( nombre_producto ), (SELECT sum(cantidad_producto) from detalles_pedidos inner join img_productos on img_productos.id_img_producto = detalles_pedidos.id_img_producto where id_producto = productos.id_producto ) cantidad_producto from detalles_pedidos inner join img_productos on img_productos.id_img_producto = detalles_pedidos.id_img_producto inner join productos on productos.id_producto = img_productos.id_producto order by cantidad_producto desc limit 10;", null );
