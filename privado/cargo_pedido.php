...
//Se establece todas las clases o funciones necesarias ='D'
	...
	//Se llama a la funcion header que pone todos los css y para todos los .php de publico ='}
  	$permisos = Page::header("Cargo por pedido", "datos");
  if(!empty($_POST)){
  	try{
	    $action = $_POST['action'];
	    if ( $action != "Buscar" ){
	    	$cargo_pedido = $_POST['txtpro_min'];
	    	
		    require("../sql/conexion.php");
		    $PDO->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
		   	//Validar
		   	//require("../lib/validator.php");
		   	if (Validator::numero( 	$cargo_pedido ) ){
			  
			  	if ( ! Validator::permiso_modificar( $permisos ) ) $error[] = "No tiene permisos para realizar esta accion.";
			  	else
			  	{
			  	
				        $sql = "UPDATE datos set cargo_pedidos=? where id_dato=?";
				        $stmt = $PDO->prepare($sql);
				        $exito = $stmt->execute(array ( $cargo_pedido,1 ) );
			...
			( ! ( Validator::numero($cargo_pedido ) ) ) ? $error[] = "Cargo_pedido: Solo se permite numeros" : "";
			}	
		}
   ...
   $palabra = '';
	...
	print ( ( isset($exito) && $exito == 1 ) ? '<div class="alert alert-success" role="alert">Cargo '.$palabra.' con exito.</div>' : "" );
...
?>
		            			...
		            			<div class="col-xs-7 col-sm-6 col-sm-offset-0 col-md-7 col-lg-6  no_padding margenredes">
									<div class="col-xs-3 col-sm-2 col-md-offset-1 col-md-3 col-lg-3 col-lg-offset-0 ">
										<label class="">Cargo por pedido:</label>
									</div>
						...
						<div class="box-body table-responsive">
							<div class="col-sm-12 col-md-12 col-lg-12">
								<table class="table table-bordered table-hover conf_tabla">
								    <thead>
								        <tr>
								            <th class="col-sm-3 col-md-3 col-lg-3">Cargo por pedido</th>
								    ...
								    <?php
									    ...
									    $tabla.="</div>";
									    $tabla.="<td><div class='pro_min'>$datos[cargo_pedidos]</div></td>";
									    $tabla.="<td> ";
