<?php
	...
	//Se llama a la funcion header que pone todos los css y para todos los .php de publico ='}
  	...
  	if ( $permisos == 0 ) header( "location: index" );
	            				...
	            				$productos = Database::getRows( "SELECT DISTINCT(nombre_producto), (SELECT COUNT(id_comentario) FROM comentarios_productos WHERE id_producto = productos.id_producto) cantidad_comentario FROM productos, comentarios_productos WHERE productos.id_producto = comentarios_productos.id_producto ORDER BY cantidad_comentario DESC LIMIT 10;", null );
	            				if ( isset( $_POST['fecha1'] ) && isset( $_POST['fecha2'] ) ){
	            					if ( Validator::fecha( $_POST['fecha1'] ) ) {
		            					...
		            					if ( Validator::fecha( $_POST['fecha2'] ) ){
		            						if ( Date( $_POST['fecha2'] ) >= Date( $_POST['fecha1'] ) ) $productos = Database::getRows( "SELECT DISTINCT(nombre_producto), (SELECT COUNT(id_comentario) FROM comentarios_productos WHERE id_producto = productos.id_producto and fecha_ingreso BETWEEN '2016-09-14' and '2016-09-17') cantidad_comentario FROM productos, comentarios_productos WHERE productos.id_producto = comentarios_productos.id_producto ORDER BY cantidad_comentario DESC LIMIT 10;", array( $_POST['fecha1'], $_POST['fecha2']  ) );
	            				...
	            				foreach ( $productos as $producto ) {
	            					...
	            					$c[] = $producto['cantidad_comentario'];
	            				}
